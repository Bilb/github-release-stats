...
function showStats(data) {
  ...
  if (err) {
    html =
      "<div class='col-md-6 col-md-offset-3 error output'>" +
      errMessage +
      '</div>';
  } else {
    ...
    $.each(data, function (index, item) {
      ...
      if (latest) {
        html +=
          "<div id='latest' class='row release latest-release'>" +
          "<h2><a href='" +
          releaseURL +
          "' target='_blank'>" +
      ...
      if (hasAssets) {
        ...
        $.each(releaseAssets, function (index, asset) {
          ...
          var assetSize = (asset.size / 1048576.0).toLocaleString(undefined, {
        ...
        downloadInfoHTML =
          '<center><i>This release has no download assets available!</i></center>';
      }

      html +=
        "<h4><span class='glyphicon glyphicon-info-sign'></span>&nbsp&nbsp" +
        'Release Info:</h4>';

      html += '<ul style="list-style-type:none">';

      html +=
        "<li><span class='glyphicon glyphicon-calendar'></span>&nbsp&nbspPublished on: " +
        publishDate +
        '</li>';

      if (hasAuthor) {
        html +=
          "<li><span class='glyphicon glyphicon-user'></span>&nbsp&nbspRelease Author: " +
          "<a href='" +
      ...
      if (hasAssets) {
        html +=
          "<li><span class='glyphicon glyphicon-download'></span>&nbsp&nbspDownloads (without .yml, .blockmap & .asc): " +
          ReleaseDownloadCount.toLocaleString() +
          '</li>';
      }
    ...
    if (totalDownloadCount > 0) {
      ...
      totalHTML +=
        "<h2><span class='glyphicon glyphicon-download'></span>" +
...
function getStats() {
  ...
  var url =
    apiRoot + 'repos/' + user + '/' + repository + '/releases?per_page=1000';
